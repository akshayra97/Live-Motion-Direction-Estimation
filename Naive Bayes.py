...
# The images are separated into folders based on their labels
...
num_up = labels.count(0)
num_down = labels.count(1)
num_left = labels.count(2)
num_right = labels.count(3)
num_forward = labels.count(4)
num_backward = labels.count(5)

# Naive Bayes Model
# Split the images into training sets and testing sets
X_train, X_test, y_train, y_test = train_test_split(dispars, labels, test_size=0.2, random_state=15)
